...
def path_to_cache_dir(path):
    """
    Convert an absolute path to a directory name for use in a image_cache.

    The algorithm used is:

    #. On Windows, any ``':'`` in the drive is replaced with ``'---'``.
    #. Any occurrence of ``os.sep`` is replaced with ``'--'``.
    #. ``'.image_cache'`` is appended.
    """
    d, p = os.path.splitdrive(os.path.abspath(path))
    if d:
        d = d.replace(':', '---')
    p = p.replace(os.sep, '--')
    return d + p + '.image_cache'
...
class Cache(object):
    """
    A class implementing a image_cache for resources that need to live in the file system
    ...
    def __init__(self, base):
        ...
        :param base: The base directory where the image_cache should be located.
        """
        # we use 'isdir' instead of 'exists', because we want to
        # fail if there's a file with that name
    ...
    def prefix_to_dir(self, prefix):
        """
        Converts a resource prefix to a directory name in the image_cache.
        """
