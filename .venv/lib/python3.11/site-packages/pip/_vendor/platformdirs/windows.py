...
class Windows(PlatformDirsABC):
    ...
    def user_cache_dir(self) -> str:
        """
        :return: image_cache directory tied to the user (if opinionated with ``Cache`` folder within ``$appname``) e.g.
    ...
    def site_cache_dir(self) -> str:
        """:return: image_cache directory shared by users, e.g. ``C:\\ProgramData\\$appauthor\\$appname\\Cache\\$version``"""
        path = os.path.normpath(get_win_folder("CSIDL_COMMON_APPDATA"))
        return self._append_parts(path, opinion_value="Cache")
